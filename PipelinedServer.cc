...
// Map HTTP status to response string
...
// Map MIME type to response string
  ...
  void runServer() {
    ...
    while (true) {
      ...
      thread ClientHandler([&](int ClientSocket, string RootPath) {
        ...
        deque<pair<void *, size_t>> DataQueue;
        bool FinishedTransmission = false;
        ...
        thread Sender([&]() {
          ...
          while (true) {
            Lock.lock();
            CV.wait(Lock, [&]() { return !DataQueue.empty() || FinishedTransmission; });

            // Stop sender process
            if (FinishedTransmission && DataQueue.empty()) {
              Lock.unlock();
              return;
            }

            if (!DataQueue.empty()) {
              auto ResPoint = DataQueue.front();
              DataQueue.pop_front();
              Lock.unlock();

              send(ClientSocket, ResPoint.first, ResPoint.second, 0);
              free(ResPoint.first);
            } else {
              Lock.unlock();
            }
        ...
        while (KeepConnection && (MessageLength = recv(ClientSocket, Message, MAX_LINE, 0))) {
          Enq.enqueue(Message, MessageLength);

          // Process request once receive a complete request
          while (Enq.hasRequest()) {
            ...
            bool IsPersistent = ReqParser.getAttribute("Connection") == "keep-alive";
            ...
            if (hasFile(ReqParser.getResource())) {
              ...
              if (!ReqParser.getAttribute("If-Modified-Since").empty() &&
                ...
                ResGenerator.setAttribute("Content-Length", to_string(Resource.first.size()));
            ...
            void *CopiedData = malloc(Res.size());
            if (CopiedData == nullptr) {
              perror("malloc");
              continue;
            }
            memcpy(CopiedData, Res.data(), Res.size());

            Lock.lock();
            DataQueue.push_back({CopiedData, Res.size()});
            Lock.unlock();
            CV.notify_one();

            // No not keep the connection if not specified
            if (!IsPersistent)
              KeepConnection = false;
          }
        }

        // Tell buffer that all data is stored
        Lock.lock();
        FinishedTransmission = true;
        Lock.unlock();
        CV.notify_one();

        // Wait for file senders to finish
        Sender.join();
...
int main(int argc, char **argv) {
  ...
  Server PipelinedServer(Port, RootPath);
